...
hotml_server_Main.prototype = {
	onConnect: function(ws) {
		var _gthis = this;
		console.log("src/hotml/server/Main.hx:39:","Client connected");
		...
		ws.on("close",function(err) {
			console.log("src/hotml/server/Main.hx:54:","Client disconnected");
	...
	,reload: function() {
		...
		if(this.file == null) {
			...
			console.log("src/hotml/server/Main.hx:73:","" + this.buildDir + "/" + this.scriptName + " cached");
			console.log("src/hotml/server/Main.hx:74:","Js type: " + this.file.getJsType());
			return;
		}
		var newFile = new hotml_server_Parser(data);
		var arr = this.file.makeDiffTo(newFile);
		if(hotml_server_Main.logResult) {
			console.log("src/hotml/server/Main.hx:79:",arr);
		}
		this.broadcast(arr);
		this.file = newFile;
	}
...
hotml_server_Parser.prototype = {
	...
	,parseRegular: function(line) {
		...
		if(this.matchParent.match(line)) {
			...
			if((__map_reserved[parent] != null ? _this.getReserved(parent) : _this.h[parent]) == null) {
				if(hotml_server_Parser.logSkips) {
					console.log("src/hotml/server/Parser.hx:93:","Skip " + this.currentClass.name + " parent " + parent);
				}
				return;
			}
		...
		if(StringTools.endsWith(line,".prototype = {") || line.indexOf(".prototype = $extend(") != -1) {
			if(hotml_server_Parser.logTypes) {
				console.log("src/hotml/server/Parser.hx:85:","" + this.currentClass.name + " {");
			}
			this.mode = 1;
			return;
		}
		...
		if(this.matchStaticVar.match(line)) {
			...
			if(hotml_server_Parser.logTypes) {
				console.log("src/hotml/server/Parser.hx:85:","" + className + "." + field + " = " + this.minString(value));
			}
		...
		if(this.matchStaticArr.match(line)) {
			...
			if(hotml_server_Parser.logTypes) {
				console.log("src/hotml/server/Parser.hx:85:","" + className1 + "." + field1 + " = " + value1);
			}
		...
		if(this.matchStaticFunc.match(line)) {
			...
			if(hotml_server_Parser.logTypes) {
				console.log("src/hotml/server/Parser.hx:85:","" + className2 + "." + name2 + "(" + args + ") {" + body.split("\n").length + "}");
			}
			if(hotml_server_Parser.logBodies) {
				console.log("src/hotml/server/Parser.hx:89:",body);
			}
	...
	,setConstructor: function(name,args) {
		...
		if(hotml_server_Parser.logTypes) {
			console.log("src/hotml/server/Parser.hx:85:","" + name + "(" + Std.string(args) + ").new {" + constructor.body.split("\n").length + "}");
		}
		if(hotml_server_Parser.logBodies) {
			console.log("src/hotml/server/Parser.hx:89:",constructor.body);
		}
		var _this = this.classes;
	...
	,setObj: function(name) {
		...
		if(hotml_server_Parser.logTypes) {
			console.log("src/hotml/server/Parser.hx:85:","Class " + name + " {}");
		}
		var _this = this.classes;
		if(__map_reserved[name] != null) {
			_this.setReserved(name,klass);
	...
	,setNameId: function(name,nameId) {
		...
		if((__map_reserved[name] != null ? _this.getReserved(name) : _this.h[name]) == null) {
			if(hotml_server_Parser.logSkips) {
				console.log("src/hotml/server/Parser.hx:93:","Skip " + name + " id " + nameId);
			}
			return;
		}
		var _this1 = this.classes;
	...
	,parseMethods: function(line) {
		if(this.matchFunc.match(line)) {
			...
			var body = this.readFunctionBody("\t}");
			if(hotml_server_Parser.logTypes) {
				console.log("src/hotml/server/Parser.hx:85:","function " + name + "(" + args + ") {" + body.split("\n").length + "}");
			}
			if(hotml_server_Parser.logBodies) {
				console.log("src/hotml/server/Parser.hx:89:",body);
			}
	...
	,compareClass: function(klass,result) {
		...
		if(old == null) {
			console.log("src/hotml/server/Parser.hx:302:","New class: " + className);
			result.push({ type : "addClass", klass : klass});
			return;
		}
		if(old.parent != klass.parent) {
			console.log("src/hotml/server/Parser.hx:307:","" + className + ": new parent: " + klass.parent);
			result.push({ type : "addClass", klass : klass});
			return;
		}
		...
		while(_g < keys.length) {
			...
			if((__map_reserved[key1] != null ? _this1.getReserved(key1) : _this1.h[key1]) != newValue) {
				console.log("src/hotml/server/Parser.hx:316:","" + className + ": static var " + key1 + " value: " + this.minString(newValue));
		...
		while(_g1 < keys1.length) {
			...
			if("" + Std.string(value) != "" + Std.string(newValue1)) {
				console.log("src/hotml/server/Parser.hx:325:","" + className + ": func " + key2 + "() value: " + this.minString("" + Std.string(newValue1)));
				if(newValue1 == null) {
	...
	,compareEnum: function(en,result) {
		...
		var old = __map_reserved[key] != null ? _this.getReserved(key) : _this.h[key];
		if(old == null) {
			console.log("src/hotml/server/Parser.hx:342:","New enum: " + en.nameId);
			result.push({ type : "addEnum", enumeration : en});
			return;
		}
		if(old.body != en.body) {
			console.log("src/hotml/server/Parser.hx:347:","New enum body: " + this.minString(en.body));
			result.push({ type : "addEnum", enumeration : en});
			return;
		}
	}
